export function useAudioPlayer() {
  ...
  const { getTrackAudioBlobUrl, isTrackDownloaded } = useOffline();

  async function loadTrack(track: PlayableTrack, crossfade = false) {
    let url = getStreamUrl(track.streamUrlId!);

    // If track is downloaded offline, play directly from local IndexedDB blob
    if (isTrackDownloaded(track.id)) {
      try {
        const offlineUrl = await getTrackAudioBlobUrl(track.id);
        if (offlineUrl) {
          url = offlineUrl;
        }
      } catch (err) {
        console.warn('Failed to load offline blob for track:', err);
      }
    }
    ...
    // No preloaded element, so fetch from stream URL / local Blob URL.
    if (crossfade) {
  ...
  function getFrequencyData(array: Uint8Array) {
    if (audioPlayer.value) {
      audioPlayer.value.getByteFrequencyData(array);
    } else {
      array.fill(0);
    }
  }
