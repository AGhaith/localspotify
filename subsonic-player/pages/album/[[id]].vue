...
const albumDownloadProgress = computed(() => {
  if (!albumData.value.album) return undefined;
  return getAlbumDownloadProgress(albumData.value.album.id);
});

const isDownloaded = computed(() => {
  if (!albumData.value.album) return false;
  return isAlbumDownloaded(albumData.value.album.id);
});

async function toggleDownloadAlbum() {
  if (!albumData.value.album) return;
  if (isDownloaded.value) {
    await removeDownloadedAlbum(albumData.value.album.id);
  } else {
    await downloadAlbum(albumData.value.album);
  }
}
    ...
    <div v-if="albumData.album" ref="albumContent">
      ...
      >
        ...
        <div class="list">
          ...
          <!-- Spotify-Style Offline Download Album Button -->
          <ButtonLink
            class="neoButton"
            :class="{ 'neoButtonPrimary': isDownloaded, 'animate-pulse': albumDownloadProgress?.isDownloading }"
            :disabled="!albumData.album.tracks.length || albumDownloadProgress?.isDownloading"
            :icon="isDownloaded ? ICONS.downloaded : (albumDownloadProgress?.isDownloading ? ICONS.spinner : ICONS.download)"
            :title="isDownloaded ? 'Downloaded (Click to remove)' : 'Download Album for offline'"
            @click="toggleDownloadAlbum"
          >
            <template v-if="albumDownloadProgress?.isDownloading">
              Downloading {{ albumDownloadProgress.current }}/{{ albumDownloadProgress.total }} ({{ albumDownloadProgress.progress }}%)
            </template>
