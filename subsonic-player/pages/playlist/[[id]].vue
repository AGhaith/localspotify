...
const downloadProgress = computed(() => {
  ...
  return getPlaylistDownloadProgress(playlist.value.id);
});

const isDownloaded = computed(() => {
  if (!playlist.value) return false;
  return isPlaylistDownloaded(playlist.value.id);
});

async function toggleDownloadPlaylist() {
  if (!playlist.value) return;
  if (isDownloaded.value) {
    await removeDownloadedPlaylist(playlist.value.id);
  } else {
    await downloadPlaylist(playlist.value);
  }
}
</script>
    ...
    <div v-if="playlist" ref="playlistContent">
      ...
      >
        ...
        <div class="list">
          ...
          <!-- Spotify-Style Offline Download Button -->
          <ButtonLink
            class="neoButton"
            :class="{ 'neoButtonPrimary': isDownloaded, 'animate-pulse': downloadProgress?.isDownloading }"
            :disabled="!hasTracks || downloadProgress?.isDownloading"
            :icon="isDownloaded ? ICONS.downloaded : (downloadProgress?.isDownloading ? ICONS.spinner : ICONS.download)"
            :title="isDownloaded ? 'Downloaded (Click to remove)' : 'Download Playlist for offline'"
            @click="toggleDownloadPlaylist"
          >
            <template v-if="downloadProgress?.isDownloading">
              Downloading {{ downloadProgress.current }}/{{ downloadProgress.total }} ({{ downloadProgress.progress }}%)
            </template>
